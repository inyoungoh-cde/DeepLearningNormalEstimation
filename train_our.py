...
import MLCPNet_our_v2 as DeepFit

from dataset_cd import PointcloudPatchDataset, RandomPointcloudPatchSampler, SequentialShapeRandomPointcloudPatchSampler, SequentialPointcloudPatchSampler

def parse_arguments():
    parser = argparse.ArgumentParser()

    # naming / file handling
    parser.add_argument('--name', type=str, default='test_LAB12345+GAB_order4', help='training run name')
    ...
    parser.add_argument('--logdir', type=str, default='./log_ablation_noLw/', help='training log folder')
    ...
    parser.add_argument('--scheduler_type', type=str, default='step', help='step or or cosine or plateau')
...
def train_pcpnet(opt):
    ...
    os.system('cp train.py %s' % (log_dirname))  # backup the current training file

    if opt.refine:
        refine_model_filename = os.path.join(out_dir, '{}_model_{}.pth' .format(opt.name, opt.refine_epoch))
        print("####REFINE(dif/name/epoch) :", out_dir,"   ",opt.name,"    ",opt.refine_epoch,"\n")
    ...
    log_string("===number of trainable parameters in upsampler: {:.4f} M === ".format(float(total_trainable_parameters / 1e6)), log_file)
    ...
    if opt.optimizer == 'SGD':
        optimizer = optim.SGD(model.parameters(), lr=100*opt.lr, momentum=opt.momentum) # adam lr=0.0005 || SGD lr=0.1
    ...
    if opt.scheduler_type == 'step':
        ...
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, opt.nepoch, eta_min=opt.lr/100) # eta_min generally equals to lr/100 with SGD
    else:
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            # forward pass
            pred, weights, trans, trans2, neighbor_normals, bias, points_new = model(points)
            spts=points[:,:,:weights.size()[1]]

            loss, n_loss, _, consistency_loss, normal_loss, bias_loss, weight_gudiance_loss, z_trans_loss = compute_loss(
                ...
                opt=opt, trans=trans, trans2=trans2, bias=bias, pcl_in=spts)
                #opt=opt, trans=trans, trans2=trans2, bias=bias, pcl_in=points_new)
            ...
            train_writer.add_scalar('consistency_loss', consistency_loss.item(),
                                    (epoch + train_fraction_done) * train_num_batch * opt.batchSize)
            #train_writer.add_scalar('weight_loss', weight_loss.item(),
            #                        (epoch + train_fraction_done) * train_num_batch * opt.batchSize)
            train_writer.add_scalar('normal_loss', normal_loss.item(),
                                    (epoch + train_fraction_done) * train_num_batch * opt.batchSize)
            test_writer.add_scalar('weight_loss', weight_gudiance_loss.item(),
                                   (epoch + test_fraction_done) * train_num_batch * opt.batchSize)
            test_writer.add_scalar('z_trans_loss', z_trans_loss.item(),
                                   (epoch + test_fraction_done) * train_num_batch * opt.batchSize)  
            ...
            while test_fraction_done <= train_fraction_done and test_batchind+1 < test_num_batch:
                ...
                loss, n_loss, err_angle, consistency_loss, normal_loss, bias_loss, weight_gudiance_loss, z_trans_loss = compute_loss(
                    pred=pred, target=target,
                    outputs=opt.outputs,
                    output_pred_ind=output_pred_ind,
                    output_target_ind=output_target_ind,
                ...
                test_writer.add_scalar('consistency_loss', consistency_loss.item(),
                                       (epoch + test_fraction_done) * train_num_batch * opt.batchSize)
                #test_writer.add_scalar('weight_loss', weight_loss.item(),
                #                       (epoch + train_fraction_done) * train_num_batch * opt.batchSize)
...
def compute_loss(pred, target, outputs, output_pred_ind, output_target_ind, output_loss_weight, normal_loss_type, arch,
                 ...
                 use_consistency=False, point_weights=None, neighbor_normals=None, opt=None, trans=None, trans2=None, bias=None, pcl_in=None):
    ...
    weight_gudiance_loss = torch.zeros(1, device=pred.device, dtype=pred.dtype)  # 새롭게 추가된 부분
    z_trans_loss = torch.zeros(1, device=pred.device, dtype=pred.dtype)  # 새롭게 추가된 부분
    additional_loss = torch.zeros(1, device=pred.device, dtype=pred.dtype)  # 새롭게 추가된 부분
    mB,mN=point_weights.size()
    ...
    for oi, o in enumerate(outputs):
        if o == 'unoriented_normals' or o == 'oriented_normals':
            ...
            if patch_rot is not None:
                ...
                # 추가된 weight_loss 부분
            if pcl_in is not None and point_weights is not None:
                Tpcl_in=pcl_in.transpose(1, 2)
                thres_d = 0.05 ** 2
                Tpcl_in = Tpcl_in.transpose(1, 2)  # (B, N, C) -> (B, C, N)로 변환
                #print(Tpcl_in.shape)
                #print("HERE:  ",o_target.shape)
                normal_dis = torch.bmm(o_target.unsqueeze(1), Tpcl_in).pow(2).squeeze()  # 거리 계산
                sigma = torch.mean(normal_dis, dim=1) * 0.3 + 1e-5  # Sigma 계산
                sigma = torch.where(sigma < thres_d, torch.ones_like(sigma) * thres_d, sigma)
                true_weight = torch.exp(-normal_dis / sigma.unsqueeze(-1))  # 진짜 가중치 계산
                weight_gudiance_loss = (true_weight - point_weights).pow(2).mean()  # 가중치 손실 계산
                additional_loss += weight_gudiance_loss  # 최종 손실에 추가
            if trans is not None:
                z_vector = torch.tensor([0, 0, 1], dtype=torch.float32).repeat(mB, 1).to(trans.device)  # Z 벡터 생성
                z_vector_rot = torch.bmm(z_vector.unsqueeze(1), trans.transpose(2, 1)).squeeze(1)  # Z 벡터 회전
                z_vector_rot = F.normalize(z_vector_rot, dim=1)  # 정규화
                z_trans_loss = 0.5 * torch.norm(torch.cross(z_vector_rot, o_target, dim=-1), p=2, dim=1).mean()  # Z 변환 손실 계산
                additional_loss += z_trans_loss  # 최종 손실에 추가
            ...
            if use_consistency:
                ...
                loss = consistency_loss + normal_loss + bias_loss + additional_loss
        else:
            raise ValueError('Unsupported output type: %s' % (o))

    return loss, n_loss, err_angle, consistency_loss, normal_loss ,bias_loss, weight_gudiance_loss, z_trans_loss
...
def get_model(opt, log_dirname):
    ...
    os.system('cp MLCPNet.py %s' % (log_dirname))
    return model
