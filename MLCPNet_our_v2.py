...
class AdaptiveLayer(nn.Module): #newly -added from https://github.com/UestcJay/GraphFit/blob/master/models/GraphFit_ms.py
...
class PointNetFeatures(nn.Module):
    def __init__(self, num_points=500, num_scales=1, use_point_stn=False, use_feat_stn=False, point_tuple=1, sym_op='max', k=20):
        ...
        self.att_layer = AdaptiveLayer(64)
    ...
    def forward(self, x):
        ...
        x_ver2, x_fea2, x_glo2, _ = atten_get_graph_feature(points, k=self.k*2, firstlayer=True)  # x_glo:(256,512)
        x2 = self.graph_attention1(x_ver2, x_fea2)
        #self.k	20
        #points	[64,3,500]
        #x_ver1	[64,3,500,1]
        #x_fea1	[64,10,500,20]
        ...
        #print("self.k: ",self.k,"   : ",points.shape,"  ",x_ver1.shape,"  ",x_fea1.shape,"  ",x_glo.shape,"  -=-=>  ",x1.shape)
        #print("self.k: ",self.k,"   : ",points.shape,"  ",x_ver2.shape,"  ",x_fea2.shape,"  ",x_glo2.shape,"  -=-=>  ",x2.shape,"\n")

        #x = F.relu(self.bn1(self.conv1(x)))
        x = F.relu(self.bn2(self.conv2(x1)))
        xx = F.relu(self.bn2(self.conv2(x2)))
        x=self.att_layer(x,xx)
        x1=self.att_layer(x1,x2)
...
class GAB(nn.Module):  # Global Attention Block
    ...
    def forward(self, x_query, x_key):
        ...
        scores = F.leaky_relu(self.bn3(self.conv3(features)), negative_slope=0.2)
        scores = self.dp3(scores)
        scores = F.leaky_relu(self.bn4(self.conv4(scores)), negative_slope=0.2)
        scores = self.dp4(scores)  # Attention score
        scores = scores.squeeze(1)
        scores = F.softmax(scores, dim=1) # (B,1)
        scores = scores.unsqueeze(-1).repeat(1, 1, 2*n_pts)
        feature = values.mul(scores.transpose(2, 1))
...
class DeepFit(nn.Module):
    ...
    def forward(self, points):
        ...
        neighbor_normals = self.mlp_nn(feat.transpose(2, 1))             # (B, n, 3)
        neighbor_normals = F.normalize(neighbor_normals, p=2, dim=-1)

        return normal, weights.squeeze(), trans, trans2, neighbor_normals, bias, points
