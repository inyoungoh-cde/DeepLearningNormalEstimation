...
def parse_arguments():
    ...
    parser.add_argument('--models', type=str, default='test_GAB_RPE', help='names of trained models, can evaluate multiple models')
...
def test_n_est(opt):
    ...
    if opt.batchSize == 0:
        ...
        if trainopt.arch == 'simple':
            spec = importlib.util.spec_from_file_location("AdaFit_multi_scale",  "MLCPNet_our_v2.py")
        ...
        for batchind, data in batch_enum:
            ...
            with torch.no_grad():
                if trainopt.arch == 'simple' or trainopt.arch == 'res' or trainopt.arch == '3dmfv':
                    ...
                    n_est, beta_pred, weights, trans, trans2, neighbor_normals,_, _ = regressor(points)
                    end_time = time.time()
